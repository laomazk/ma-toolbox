...
public class SpringCacheAnnotationParser implements CacheAnnotationParser, Serializable {
    ...
    static {
        ...
        CACHE_OPERATION_ANNOTATIONS.add(ExCaffeine.class);
    }
    ...
    private Collection<CacheOperation> parseCacheAnnotations(
        ...
        // 添加自定义注解
        anns.stream().filter(ann -> ann instanceof ExCaffeine).forEach(
                ann -> ops.add(parseCaffeineConfigAnnotation(ae, cachingConfig, (ExCaffeine) ann)));
        return ops;
    }

    private CacheableOperation parseCaffeineConfigAnnotation(AnnotatedElement ae, DefaultCacheConfig defaultConfig, ExCaffeine config) {
        CacheableOperation.Builder builder = new CacheableOperation.Builder();

        builder.setName(ae.toString());
        builder.setCacheNames(config.cacheNames());
        builder.setCondition(config.condition());
        builder.setUnless(config.unless());
        builder.setKey(config.key());
        builder.setKeyGenerator(config.keyGenerator());
        builder.setCacheManager(config.cacheManager());
        builder.setCacheResolver(config.cacheResolver());
        builder.setSync(config.sync());
